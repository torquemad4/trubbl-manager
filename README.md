...
- **Forfeits and concessions.** Rule from the portal or with `/trubbl forfeit`.
  A ruling overrides TourPlay, survives every later sync, and is reversible —
  the superseded ruling is kept, not deleted.
- **Standings.** Built from TourPlay results plus your rulings, with forfeits
  given tracked per team so repeat offenders are visible.
...
Open the portal and give it the slug `trubbl-season-7`. That imports coaches,
teams, rounds and fixtures. Then go to Rounds, set a start date and a window
length, and apply it across the season.
...
| `/trubbl forfeit match:<id> side:<home\|away\|both>` | admin role | Rule a forfeit |

## Settings worth understanding

**`auto_forfeit_on_close`** is `false` by default. Turned on, a window expiring
with a game unplayed and no extension granted rules a **double forfeit** — not a
one-sided one. At that moment nobody has shown which coach was at fault, and
guessing would put a loss on someone who may have been chasing an opponent all
month. Re-rule it by hand once you know more; the double forfeit is reversible.
...
src/rules.ts      windows, forfeit scoring, standings — pure, and tested
